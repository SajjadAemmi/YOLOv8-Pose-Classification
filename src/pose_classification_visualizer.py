        ...
        for classification_history in [self._pose_classification_history,
            ...
            for classification in classification_history:
                if classification is None:
                    ...
                    y.append(0)
            plt.plot(y, linewidth=7, label='Filtered' if classification_history == self._pose_classification_filtered_history else 'Raw')

        plt.grid(axis='y', alpha=0.75)
        plt.xlabel('Frame')
        plt.ylabel('Confidence')
        plt.title(f'Classification history for `{self._class_name}`')
